<script>
  ...
  function calculateTotals() {
    currentQuotation.subtotal = quoteItems.reduce((s, i) => s + i.total, 0);
    currentQuotation.total    = currentQuotation.subtotal - (Number(currentQuotation.discount) || 0);
  }

  function addItemToQuote(item) {
    const qty = parseInt(addQty[item.id]) || 1;
    const existing = quoteItems.find(q => !q.is_package && q.id === item.id);
    if (existing) {
  ...
  function addPackageToQuote(pkg) {
    const existing = quoteItems.find(q => q.is_package && q.id === pkg.id);
    if (existing) {
      existing.quantity += 1;
      existing.total     = existing.quantity * existing.price;
      quoteItems = [...quoteItems];
  ...
  function updateLine(i) {
    quoteItems[i].total = quoteItems[i].quantity * quoteItems[i].price;
    quoteItems = [...quoteItems];
    calculateTotals();
  }
  ...
  async function generatePDF() {
    if (!currentQuotation.client_id) { alert('Seleccione un cliente primero.'); return; }
    calculateTotals();
  ...
  async function saveQuotation() {
    if (!currentQuotation.client_id) { alert('Seleccione un cliente.'); return; }
    isSaving = true;
    calculateTotals();
    try {
      let qId;

<script>
  /**
   * Estado vacio con las cuatro piezas que pide el sistema de diseño: icono de
   * trazo, titulo, una linea de explicacion y el boton que lo resuelve.
   *
   * Venia de la ficha de Cloud y sube aqui para que las DOS apps enseñen el
   * mismo vacio. La version que habia en este paquete era un `<div>` con un
   * mensaje y la usaba una sola pantalla de pruebas; su API sigue funcionando
   * —`message` y el slot— para no romperla.
   *
  ...
  /** Compatibilidad con la version anterior: un mensaje suelto, sin adornos. */
  export let message = '';

  // Sin titulo ni icono explicito, se comporta como el componente de antes: solo
  // el texto centrado. Asi la pantalla de pruebas sigue viendose igual.
  $: simple = !title;
</script>

{#if simple}
  <div class="empty-state">
    <slot>{message || 'No hay datos disponibles.'}</slot>
  </div>
{:else}
  <div class="empty">
...
<style>
  ...
  /* Delta sobre `.empty-state-icon` de theme.css, que esta a 3rem porque nacio

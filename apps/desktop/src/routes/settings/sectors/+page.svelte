    ...
    try {
      if (isEditing) {
        await window.api.db.run(
          'UPDATE commercial_sectors SET name = ?, description = ? WHERE id = ?',
          [current.name.trim(), current.description || null, current.id]
        );
      } else {
        await window.api.db.run(
          'INSERT INTO commercial_sectors (name, description) VALUES (?, ?)',
  ...
  <div class="table-wrapper">
    <table class="table">
      ...
      <tbody>
        ...
        {:else}
          <tr>
            <td colspan="3" style="text-align: center; color: var(--text-muted); padding: 30px;">No hay sectores registrados.</td>
          </tr>
        {/each}
      </tbody>
    </table>
...
<Modal bind:show={showModal} title={isEditing ? 'Editar entrada' : 'Nuevo Sector'} maxWidth="480px">
  <div style="display: flex; flex-direction: column; gap: 16px;">
    <div>
      <label for="cat-name">Nombre *</label>
      <input id="cat-name" type="text" bind:value={current.name} class="form-control" placeholder="Ej. Eventos, Hoteles, Retail..." />
    </div>
    <div>

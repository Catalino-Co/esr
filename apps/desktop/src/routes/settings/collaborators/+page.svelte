<script>
  ...
  async function loadData() {
    if (window.api && window.api.db) {
      ...
      collaborators = await window.api.db.get(
        `SELECT * FROM collaborators WHERE ${where.join(' AND ')} ORDER BY name ASC`,
        params
      );
    }
  }
...
<div class="herramientas">
  ...
  <div class="herramientas-datos">
    ...
    />
    <button class="btn btn-primary btn-new" on:click={openCreate}>+ Nuevo Colaborador</button>
  </div>
</div>
...
<Modal bind:show={showModal} title={isEditing ? 'Editar Colaborador' : 'Nuevo Colaborador'}>
  ...
  <div slot="footer">
    ...
    <button class="btn btn-primary" on:click={saveCollaborator}><Icon name="check" size={16} />Guardar</button>
  </div>
</Modal>

<script>
  ...
  $: siblings = getSiblings(slug);
  $: content = getDocContent(slug);
  // Con contenido escrito el TOC sale de los encabezados del Markdown; sin el,
  // del esquema declarado en el indice.
  $: topics = content ? content.topics : (section?.topics ?? []);
</script>

<svelte:head>
	<title>{section ? `${section.title} — Documentación` : 'Sección no encontrada'} — ESR Pro</title>
</svelte:head>
	...
	<div class="doc-layout">
		<article class="card doc-article">
			...
			<div class="card-title">
				<span><span class="doc-title-icon" aria-hidden="true">{section.icon}</span> {section.title}</span>
			</div>
			<p class="doc-lead">{section.summary}</p>

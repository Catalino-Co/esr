<script>
	...
	/**
	 * Buscar un paquete por su código. Gemelo de `invoices/BuscarFactura.svelte`,
	 * sin columna de fecha: un paquete es catálogo, no un documento con fecha de
	 * operación.
	 *
	 * @type {{ onclose: () => void }}
	 */
	...
	async function buscar() {
		...
		error = '';
		try {
			const res = await fetch(`/packages/buscar?q=${encodeURIComponent(termino.trim())}`, {
				signal: control.signal
			});
			if (!res.ok) throw new Error('El servidor rechazó la búsqueda.');
			const { paquetes } = await res.json();
			resultados = paquetes;
			elegida = paquetes.length > 0 ? 0 : -1;
		} catch (/** @type {any} */ e) {
	...
	function abrir() {
		const paquete = resultados[elegida];
		if (!paquete) return;
		goto(`/packages/${paquete.id}`);
	}
	...
	$effect(() => {
		if (elegida < 0) return;
		document
			.getElementById(`resultado-paquete-${elegida}`)
			?.scrollIntoView({ block: 'nearest' });
	});
</script>

<Modal open title="Buscar paquete por código" size="md" {onclose}>
	{#if error}
	...
	<input
		...
		aria-controls="resultados-paquete"
		aria-autocomplete="list"
		aria-activedescendant={elegida >= 0 ? `resultado-paquete-${elegida}` : undefined}
		autocomplete="off"
	/>

	{#if termino.trim().length < 2}
		<p class="panel-hint">
			Escriba al menos dos caracteres del código. Se busca en todos los paquetes, también fuera
	...
	{:else}
		<ul class="resultados" id="resultados-paquete" role="listbox" aria-label="Paquetes encontrados">
			{#each resultados as paquete, indice (paquete.id)}
				<!-- svelte-ignore a11y_click_events_have_key_events -->
				<li
					id="resultado-paquete-{indice}"
					class="resultado"
					...
					ondblclick={abrir}
				>
					<span class="codigo">{paquete.code}</span>
					<span class="nombre">{paquete.name}</span>
					<span class="badge {recordStateBadgeClass(paquete.is_active)}">
						{recordStateLabel(paquete.is_active)}
					</span>
				</li>
			{/each}
		</ul>

<script>
	...
	/**
	 * Buscar una cotización por su número. Calcado de `work-orders/BuscarOrden.svelte`
	 * —mismo patrón combobox, misma razón para no ser un desplegable flotante—.
	 *
	 * @type {{ onclose: () => void }}
	 */
	let { onclose } = $props();
	...
	function abrir() {
		const quote = resultados[elegida];
		if (!quote) return;
	...
	$effect(() => {
		if (elegida < 0) return;
		document
			.getElementById(`resultado-cotizacion-${elegida}`)
			?.scrollIntoView({ block: 'nearest' });
	});
</script>

<Modal open title="Buscar cotización por número" size="md" {onclose}>
	{#if error}
	...
	<input
		...
		oninput={alTeclear}
		onkeydown={alPulsar}
		placeholder="COT-000012"
		aria-label="Número de cotización"
		aria-expanded={resultados.length > 0}
		aria-controls="resultados-cotizacion"
		aria-autocomplete="list"
		aria-activedescendant={elegida >= 0 ? `resultado-cotizacion-${elegida}` : undefined}
		autocomplete="off"
	/>

	{#if termino.trim().length < 2}
		<p class="panel-hint">
			Escriba al menos dos caracteres del número. Se busca en todas las cotizaciones, también
			fuera del rango de fechas del listado.
		</p>
	...
	{:else}
		<ul class="resultados" id="resultados-cotizacion" role="listbox" aria-label="Cotizaciones encontradas">
			{#each resultados as quote, indice (quote.id)}
				<!-- svelte-ignore a11y_click_events_have_key_events -->
				<li
					id="resultado-cotizacion-{indice}"
					class="resultado"
				...
				>
					...
					<span class="total">{formatMoney(quote.total)}</span>
